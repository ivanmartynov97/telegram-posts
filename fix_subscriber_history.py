#!/usr/bin/env python3
"""Разовый фикс: subscriber_history.json должен быть словарём {date: {count, ts}}, не списком."""
...
url = f"https://api.github.com/repos/{GH_REPO}/contents/analytics/subscriber_history.json"
...
payload = {
    "message": "Fix subscriber_history.json format: list -> dict",
...
with urllib.request.urlopen(req2, timeout=10, context=ssl_ctx()) as r:
    print("✅ Исправлено:", json.loads(r.read())["commit"]["sha"])
